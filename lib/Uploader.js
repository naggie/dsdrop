...
var Main = function() {
	...
	this.setFilepath = function(filepath) {
		...
		self.filesize = fs.statSync(filepath)['size']
	...
	this.publish = function(done) {
		...
		if (!self.filepath) return done('setFilepath() first')


		self.hashfile(function(err,hash) {
	...
	this.hashfile = function(done) {
		if (!arguments[0]) var done = function(){}
