...
var Main = function() {
	...
	this.on('packageReady'       ,hashfile)
	this.on('hashReady'          ,attemptInstantUpload)
	this.on('serverNeedsPackage' ,uploadFile)
	...
	var publish = function() {
		if (!self.token)    return emit('error',loginRequiredError())
		if (!self.url)      return emit('error',Error('Client library: URL attribute not set'))


		hashfile(function(err,hash) {
