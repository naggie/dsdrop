...
exports.add = function (file, done) {
	if ( !file.name || !file.user || !file.size)
		return done('Incomplete file object')

	if (!file.mimetype)
		file.mimetype = mimetype[file.name.match(/[a-z0-9]{1,4}$/) || 'default']

	// get token
	keygen(function(err,token) {
		if (err) return done(err)
		file.token = token
		// publish instance, and update LRU
		rclient.zadd('database:lru',unixtime(),token)
		rclient.hmset('database:'+token,file,function(err) {
			done(false,token)
		})
	})
}

// remove all files by hash
exports.remove = function (hash, done) {
