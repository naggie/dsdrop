	...
	rclient.multi()
		...
		.exec(function(err,replies) {
			...
			for (var i in bits) {
				...
				// todo auto units
				memory_used   : (info.used_memory/(1024*1024)).toPrecision(3),
				memory_limit  : config.memory_limit_MB,
				memory_units  : 'MB',

				storage_used  : (replies[1]/(1024*1024*1024)).toPrecision(3),
				storage_limit : config.storage_limit_GB,
				storage_units : 'GB',

				network_used  : (exports.throughput/(1024*1024/8)).toPrecision(3),
				network_max   : config.network_max_Mbps,
				network_units : 'Mbps',

				transferred          :  (replies[2]/(1024*1024*1024)).toPrecision(3),
