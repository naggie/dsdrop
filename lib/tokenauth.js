...
/*
   ...
   check   : retrieve username from token, if not expired
...
exports.create = function(username,context,done) {
	...
	rclient.sadd('tokenauth:usernames',username,function(err,unique) {
		...
		exports.generate_valid_unique_token(function(err,token){
			if (err) return done(err)
			rclient.set('tokenauth:tokens:'+token,username,'EX',exports.expiry,function(err) {
				done(err,token)
			})
			// record a context
			rclient.sadd('tokenauth:contexts:'+token,context,'EX',exports.expiry+1)
		})
...
exports.check = function(token,context,done) {
	rclient.get('tokenauth:tokens:'+token,function(err,username) {
		if (err) return done(err)
		if (!username) return done(err,exports.TOKEN_EXPIRED)
...
// remove all tokens for a username and attached context sets
...
exports.check('gBtnOHMO','biieiians',function(err,username) {
