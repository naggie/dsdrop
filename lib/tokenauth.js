...
exports.generate_valid_unique_key = function(done) {
	...
	// check it's unique
	rclient.sadd('token:keys',key,function(err,unique) {
...
exports.create_identity = function(p) {
	...
	rclient.sadd('token:names',p.name,function(err,unique) {
		if (!unique)
		...
		exports.generate_valid_unique_key(function(err,key){
			...
			rclient.hmset('token:identity:'+p.name,{
			...
			//rclient.hset('token:key-name',key,p.name,function(err) {
			//	p.done(err,key)
			//})
		})
...
exports.list_identities = function(done) {
	var identities = {}
	// enumerate names
	rclient.smembers('token:names',function(err,names) {
		var multi = rclient.multi()

		for (var i in names)
			multi.hgetall('token:identity:'+names[i])
...
exports.delete = function(name,done) {
	...
	rclient.srem('token:names',name)
	rclient.del('token:identity:'+name,function(err,existed) {
		if (existed)
			done(null)
		else
...
exports.regenerate = function(name,done) {
	rclient.hgetall('token:identity:'+name,function(err,identity) {
		if (identity)
			exports.generate_valid_unique_key(function(err,key){
				if (err) return done(err)

				rclient.hset('token:identity:'+name,'key',key,function(err){
					done(err,key)
				})
			})
		else
