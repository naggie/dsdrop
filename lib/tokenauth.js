...
exports.generate_valid_unique_key = function(done) {
	var key
	...
	// check it's unique
	rclient.sadd('tokenauth:tokens',key,function(err,unique) {
		if (err) console.log('error',err)

		if (unique)
			done(null,key)
		else {
			exports.generate_valid_unique_key(done)
...
exports.create_identity = function(p) {
	...
	rclient.sadd('tokenauth:names',p.name,function(err,unique) {
		if (!unique)
			return p.done('Name must be unique.')

		exports.generate_valid_unique_key(function(err,key){
			...
			rclient.hmset('tokenauth:identity:'+p.name,{
			...
			//rclient.hset('tokenauth:key-name',key,p.name,function(err) {
			//	p.done(err,key)
			//})
		})
...
exports.list_identities = function(done) {
	var identities = {}
	// enumerate names
	rclient.smembers('tokenauth:names',function(err,names) {
		var multi = rclient.multi()

		for (var i in names)
			multi.hgetall('tokenauth:identity:'+names[i])
...
exports.delete = function(name,done) {
	...
	rclient.srem('tokenauth:names',name)
	rclient.del('tokenauth:identity:'+name,function(err,existed) {
		if (existed)
			done(null)
		else
...
exports.regenerate = function(name,done) {
	rclient.hgetall('tokenauth:identity:'+name,function(err,identity) {
		if (identity)
			exports.generate_valid_unique_key(function(err,key){
				if (err) return done(err)

				rclient.hset('tokenauth:identity:'+name,'key',key,function(err){
					done(err,key)
				})
			})
		else
